...
export default function ProductsPage() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const currentPageProductIds = new Set(response.documents.map(product => product.$id));
  ...
  const toggleSelectAll = () => {
    if (!selectAllPages) {
      ...
      const currentPageProductIds = new Set(products.map(product => product.$id));
  ...
  const toggleSelectRow = (id) => {
    ...
    const currentPageProductIds = new Set(products.map(product => product.$id));
    const isAllCurrentPageSelected = [...currentPageProductIds].every(id => newSelectedRows.has(id));
    ...
    <>
      ...
      <table className="w-full border-collapse mb-4">
        ...
        <tbody>
          {products.map((product) => (
            <tr key={product.$id} className="bg-white border hover:bg-blue-100">
              ...
              <td className="border p-2 text-black max-w-[150px] truncate">{product.suppliers?.name || "Не указан"}</td>
              <td className="border p-2 text-black max-w-[150px] truncate">{product.categories?.name || "Не указана"}</td>
            </tr>
          ))}
        </tbody>
      </table>
      ...
      {showSelectionInfo && selectedRows.size > 0 && (
        <div className="fixed bottom-4 left-1/2 transform -translate-x-1/2 bg-white border border-gray-300 shadow-lg rounded-md p-4 flex justify-between items-center w-1/3">
            <span>{selectedRows.size} / {totalDocuments} записей выбрано</span>
            <button className="text-red-500" onClick={handleCancelSelection}>
            Отмена
            </button>
        </div>
        )}
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex space-x-2">
          <button
            className={`bg-gray-200 text-gray-700 p-2 rounded-lg hover:bg-gray-300 transition ${currentPage === 1 ? "cursor-not-allowed opacity-50" : ""}`}
          ...
          <button
            className={`bg-gray-200 text-gray-700 p-2 rounded-lg hover:bg-gray-300 transition ${currentPage === totalPages ? "cursor-not-allowed opacity-50" : ""}`}
            onClick={() => handlePageChange(currentPage + 1)}
